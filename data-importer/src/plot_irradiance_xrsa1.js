...
function generateHtml(datapointsAvg, datapointsMax, datapointsMin) {
    ...
    return `
        <html>
            <head>
                <title>Data Plot: XR</title>
            </head>
            <body>
                <h1 style="height: 3vh">irradiance_xrsa1</h1>
...
async function run() {
    ...
    const response = await postRequest(
        ...
        { statement: 'SELECT * FROM GOES.irradianceXrsa1.measurements' },
        dbUsername,
        dbPassword 
    );
    ...
    const datapointsAvg = response.data.results
        ...
        .map(d => {
            ...
            const reportAvg = calcSamplesAverage(d.measurements.samples);
    ...
    const datapointsMax = response.data.results
        ...
        .map(d => {
            const label = moment(d.measurements.timestamp).format('D/M H:mm');
            const reportMax = calcSamplesMax(d.measurements.samples);
    ...
    const datapointsMin = response.data.results
        ...
        .map(d => {
            const label = moment(d.measurements.timestamp).format('D/M H:mm');
            const reportMin = calcSamplesMin(d.measurements.samples);
            return { label, reportMin };
        });

    const filePath = path.join(__dirname, '..', 'out', 'plot-xr.html');
